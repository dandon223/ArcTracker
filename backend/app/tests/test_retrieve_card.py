...
class RetrieveCardAPITests(APITestCase):  # type: ignore[misc]
    ...
    def get_retrieve_card_url(self, game_id: uuid.UUID, player_id: uuid.UUID) -> str:
        return reverse("retrieve-card", kwargs={"game_id": str(game_id), "player_id": str(player_id)})
    ...
    def test_retrieve_card_post(self) -> None:
        ...
        player_hand = PlayerHand.objects.get(game=game, player_id=player_two.id)
        player_cards = player_hand.cards.all()
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(player_hand.number_of_cards, 7)
        self.assertEqual(len(player_cards), 1)
        self.assertEqual(player_cards[0].id, card.id)

    def test_retrieve_card_post_already_retrieved(self) -> None:
        game = Game.objects.first()
        assert game is not None
        game_round = GameRound.objects.first()
        assert game_round is not None
        ...
        self.assertEqual(response.data["detail"], f"card {card.id} can not be retrieved")

    def test_retrieve_card_post_new_chapter(self) -> None:
        game = Game.objects.first()
        assert game is not None
        game_round = GameRound.objects.first()
        assert game_round is not None
        ...
        response = self.client.post(self.get_retrieve_card_url(game.id, player.id), {"id": card.id})
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.data["detail"], f"card {card.id} can not be retrieved")

    def test_retrieve_card_post_new_round(self) -> None:
        game = Game.objects.first()
        assert game is not None
        game_round = GameRound.objects.first()
        assert game_round is not None
