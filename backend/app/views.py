...
def get_cards_to_play(game_object: Game, game_chapter: int, game_round: int):
    ...
    for player_object in game_object.players.all():
        player_hand_object = PlayerHand.objects.filter(game=game_object, player=player_object).get()
        for card in player_hand_object.cards.all():
            card_by_str.pop(str(card))
    ...
    game_round_objects = (
        GameRound.objects.filter(game=game_object, chapter=game_chapter).exclude(round=game_round).all()
    )
...
def get_cards_to_retrieve(game_object: Game, cards_to_play):
    ...
    if len(player_objects) == 4:
        ...
        return (
            Card.objects.exclude(Q(id__in=card_ids) | Q(number__in=[CardNumber.ONE, CardNumber.SEVEN]))
            .order_by("suit", number_order)
            .all()
        )
...
def get_card_in_hand_objects(game_object: Game):
    player_objects = game_object.players.all()
    cards_in_hand = []
    for player_object in player_objects:
        player_hand_object = PlayerHand.objects.filter(player=player_object, game=game_object).get()
        for player_card_object in player_hand_object.cards.all():
            cards_in_hand.append(player_card_object)
    card_ids = [card.id for card in cards_in_hand]
    return Card.objects.filter(id__in=card_ids).order_by("suit", number_order).all()
...
def game(
    ...
    card_in_hand_objects = get_card_in_hand_objects(game_object)
    ...
    number_of_cards_added_form = NumberOfCardsAddedForm(players=game_object.players.all())
    reveal_card_form = RevealCardForm(players=game_object.players.all(), cards=cards_to_play)
    unreveal_card_form = RevealCardForm(players=game_object.players.all(), cards=card_in_hand_objects)
    ...
    if request.method == "POST":
        ...
        if submit_type == "card_played":
            ...
            if card_played_in_round_form.is_valid():
                game_round_object = GameRound.objects.filter(
                    game=game_object, chapter=game_chapter, round=game_round
                ).get()
            ...
            number_of_cards_added_form = NumberOfCardsAddedForm(request.POST, players=game_object.players.all())
            if number_of_cards_added_form.is_valid():
                player_hand_object = PlayerHand.objects.filter(
                    player=number_of_cards_added_form.cleaned_data["player"]
                ).get()
            ...
            reveal_card_form = RevealCardForm(request.POST, players=game_object.players.all(), cards=cards_to_play)
            if reveal_card_form.is_valid():
                player_hand_object = PlayerHand.objects.filter(player=reveal_card_form.cleaned_data["player"]).get()
                player_hand_object.cards.add(reveal_card_form.cleaned_data["card"])
                player_hand_object.save()
                return redirect(f"/game/{game_id}/{game_chapter}/{game_round}")
        elif submit_type == "unreveal_card":
            unreveal_card_form = RevealCardForm(request.POST, players=game_object.players.all(), cards=card_in_hand_objects)
            if unreveal_card_form.is_valid():
                player_hand_object = PlayerHand.objects.filter(player=unreveal_card_form.cleaned_data["player"]).get()
            ...
            if CardPlayedInRound.objects.filter(player=player, game_round=game_round_object).exists():
                ...
                if card_played_in_round_object.card is not None:
                    cards_played_in_chapter[player.nick][-1].append(
                        card_played_in_round_object.card.suit + "/" + card_played_in_round_object.card.number + ".jpg"
                    )
    ...
    return render(
        ...
        "game.html",
        {
            ...
            "number_of_cards_added": number_of_cards_added_form,
            "reveal_card": reveal_card_form,
            "unreveal_card": unreveal_card_form,
        },
    )
