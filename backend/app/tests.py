...
class PlayerAPITests(APITestCase):  # type: ignore[misc]
    ...
    def test_player_post_already_created(self) -> None:
        ...
        self.assertEqual(response.data["nick"], "player with nick a already exists")
...
class GameAPITests(APITestCase):  # type: ignore[misc]
    ...
    def test_game_post_wrong_number_of_players(self) -> None:
        ...
        for game_name, player_nicks in test_cases:
            with self.subTest():
                ...
                response = self.client.post(self.url, new_data)
                self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
                self.assertEqual(response.data["players"], "game has to have from 2 to 4 players")
    ...
    def test_game_post_already_created(self) -> None:
        ...
        response = self.client.post(self.url, new_data)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.data["name"], "game with name a already exists")
    ...
    def test_game_get_no_player(self) -> None:
        response = self.client.get(self.url, {"name": "a"})
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
        self.assertEqual(response.data["name"], "game with name a does not exists")
